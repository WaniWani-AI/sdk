...
**Channel attribution.** Ingest attributes each event to a channel via `properties.channelId` or the channel's `source` tag. A host-supplied channel (`channelId` prop / `data-channel-id`) always wins; on a token-only embed the widget uses the channel the `/config` response resolves from the bearer token, so page views and host-page `track` events attribute correctly with no extra markup. If neither a channel nor a source resolves (e.g. a server whose `/config` predates the `channelId` field), the widget skips the send entirely rather than emit an event ingest is guaranteed to drop.
