...
If the installed list differs from the docs (older SDK), audit against the docs and flag the version gap as a finding: the project should upgrade (see [upgrading.md](upgrading.md) and the changelog at [docs.waniwani.ai/sdk/changelog](https://docs.waniwani.ai/sdk/changelog)).

## The taxonomy (closed set)

Snapshot for orientation; the docs pages above are authoritative. Full local reference: [events.md](events.md).
...
Do not edit code. If the user wants the gaps filled, follow [instrument-tracking.md](instrument-tracking.md) next; this report is its input.
...
```
Follow the waniwani-sdk skill's audit-tracking reference
(references/audit-tracking.md) to audit Waniwani event tracking across this MCP
project. Fetch the live event definitions from docs.waniwani.ai/sdk/tracking/events
and docs.waniwani.ai/sdk/tracking/instrumentation first. Inventory every surface,
