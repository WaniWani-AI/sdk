...
keywords: ["addNode", "interrupt node", "widget node", "action node", "flow handlers", "NodeContext"]
---
...
<Tip>
  An older positional form (`addNode(id, run, options?)`) is also accepted, but is deprecated. See [Changelog / 0.12.0](/changelog#0-12-0-addnode-object-form).
</Tip>
...
<ResponseField name="waniwani" type="ScopedWaniWaniClient | undefined">
  A session-scoped tracking client. Present when the MCP server is wrapped with [`withWaniwani()`](/configuration/wrap-server). Undefined otherwise.
</ResponseField>
...
A widget node returns `showWidget({ tool, ... })`. The engine returns `status: "widget"` pointing at a display tool (created with `createTool()`) and the flow pauses. When the user interacts, the host calls the flow back with `action: "continue"` and whatever values you want stored in `stateUpdates`.
...
<Note>
  The flow itself is a data-only tool. It never returns `structuredContent`. The display tool referenced by `showWidget` is the one that renders the widget. Register the display tool with [Skybridge's](https://docs.skybridge.tech) `server.registerWidget(name, meta, schema, handler)` and the flow with `flow.register(server)` — Skybridge handles all the widget-binding metadata for you. See the [Pet insurance example](/guides/pet-insurance#register-the-widget-display-tools) for the full pattern.
</Note>
