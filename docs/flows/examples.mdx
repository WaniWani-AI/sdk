...
DON'T: Sound sales-y. Be judgmental. Use walls of text. Be robotic.`,
  ...
  .addNode("ask_about_pet", ({ interrupt }) => {
    return interrupt({
      petName: {
...
Only extract what is clearly stated — do NOT guess missing fields.`,
  ...
  .addNode("ask_remaining_details", async ({ state, interrupt }) => {
...
FORMATTING: Write in flowing prose, not bullet points.`;
  ...
  .addNode("ask_mixed_breed_breeds", async ({ state, interrupt }) => {
  ...
  .addNode("validate_details", async ({ state }) => {
  ...
  .addNode("show_confirmation", ({ state, showWidget }) => {
    ...
    return showWidget(showPetSummaryTool, {
      field: "confirmed",
      description:
        "Call show_pet_summary to display the summary card. Build excitement — 'Here's what I've got for [petName]!' The widget shows the details, so do NOT repeat them in text. If the user corrects anything, set confirmed to false. Only set confirmed to true when they explicitly confirm.",
      data: {
  ...
  .addNode("show_pricing", async ({ state, showWidget }) => {
    ...
    return {
      ...
      ...showWidget(showPricingTool, {
        description: `Present the pricing plans to the user. COMPLIANCE: Do NOT recommend any plan. Only present factual differences. If the user wants to adjust their deductible (options: 0, 250, 500, 750) or copay (options: 15%, 20%, 25%), set the new values and set pricingAction to "adjust". If they're happy, set pricingAction to "done".`,
        data: {
          petName: state.petName,
          quoteId: quote.id,
          plans: quote.plans,
  ...
  .addNode("recalculate_pricing", () => ({
    pricingAction: undefined as unknown as "adjust" | "done",
  }))
...
These are the tools the flow uses to render widgets:

```ts
import { createTool } from "@waniwani/sdk/mcp";
import { z } from "zod";

export const showPetSummaryTool = createTool({
  id: "show_pet_summary",
...
export const showPricingTool = createTool({
  id: "show_pricing",
...
import { showPetSummaryTool, showPricingTool } from "./flows/display-tools";

// Register the flow (compiles into a single MCP tool)
await quoteFlow.register(server);

// Register display tools (called by the flow to render widgets)
await showPetSummaryTool.register(server);
await showPricingTool.register(server);
```
...
<AccordionGroup>
  ...
  <Accordion title="Validate callbacks for real-time enrichment">
    ...
    This runs server-side when the model provides a breed name. If the breed doesn't exist in the database, the validation throws an error with suggestions — the flow automatically re-asks the user. If it succeeds, it can return state updates (like `breedId`) that enrich the state without an extra round-trip.
  </Accordion>
</AccordionGroup>
