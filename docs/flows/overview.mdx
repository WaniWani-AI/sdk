...
keywords: ["flow engine", "MCP state machine", "multi-step MCP", "MCP funnel engine", "createFlow", "StateGraph"]
---
...
State persists between calls in the flow store, keyed by the session id extracted from `_meta`. You pick the store at `.compile()` time. See [KV store adapters](/flows/kv-store) and the [Platform overview](/platform/overview) for the hosted option.
...
<CardGroup cols={2}>
  <Card title="Architecture" icon="diagram-project" href="/flows/architecture">
