...
The KV store is the only place state lives between two tool calls. MCP itself is stateless: each `tools/call` is a fresh HTTP request with no memory of the previous one. Without a store, the engine has nothing to resume from.
