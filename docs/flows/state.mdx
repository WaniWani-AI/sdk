...
keywords: ["flow state", "Zod schema", "typed state", "MCP state persistence", "redacted"]
---

This page covers the state schema: the typed fields a flow collects. A flow also needs a place to persist that state between MCP calls. See [Persistence](/flows/kv-store), which is required for any flow to work.
