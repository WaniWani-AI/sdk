...
- **Project config file.** Drop a `waniwani.json` at the project root with `$schema` set to `https://app.waniwani.ai/waniwani.json`. The SDK auto-loads it on the first `waniwani()` call; the CLI uses the same file for `connect`/`dev`/evals. No explicit import required.
