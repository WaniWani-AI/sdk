...
`withWaniwani(server)` is an optional convenience that instruments an MCP server. It's free-tier-flavored when an API key is configured, and a no-op-but-still-useful wrapper when it isn't.
...
| OSS flows + hosted tracking (free tier) | Yes |
