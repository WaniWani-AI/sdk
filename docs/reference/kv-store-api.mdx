...
This page is the type-level reference for the two main extension points in the SDK: the `KvStore` interface (any flow uses it) and the `waniwani()` client (the free tier uses it).
