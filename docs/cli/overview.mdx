...
| [`waniwani connect`](#connect) | Pick an org, pick or create an agent, write the binding to `waniwani.config.ts`. |
...
3. **Write the binding.** Adds `orgId` and `projectId` to `waniwani.config.ts` at the repo root. If the file already exists, the CLI merges the keys in; if the shape isn't recognized, it prints the snippet for you to paste.
...
1. Ensures you're logged in (runs `login` if not) and that `waniwani.config.ts` has a `projectId` (runs `connect` if not).
...
- `-p, --port <port>` — Port the local MCP listens on. Defaults to `devPort` in `waniwani.config.ts`, or `3000`.
...
Shared with [`@waniwani/sdk`](/configuration/installation). `waniwani connect` writes `orgId` and `projectId`; other fields are optional and consumed by the SDK and the `dev` command:

```ts waniwani.config.ts
...
Resolution order for the API URL: env var → `waniwani.config.ts` → `.waniwani/settings.json` → default. For auth: API key → OAuth tokens (with auto-refresh).
...
# Per-project
cd my-mcp-server
waniwani connect              # creates / picks the agent, writes waniwani.config.ts
